...
public class Update extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
				...
				while(rs.next()){
			       ...
			       rs = ps.executeQuery();
			        if(rs.next()){
			        		...
			        		System.out.println(iCode+" "+iName+" "+iCost+" "+iPrice+" "+Category_Name+" "+Vendor_Name);
		...
		if(Update == true){
			System.out.println((String)request.getParameter("Category_Name").toString()+" Category name");
			int result;
			try{
				MyConn = ConnectionManager.getConnection();
				 myStmt = MyConn.createStatement();
				 
				 UpdateActor = MyConn.prepareStatement("UPDATE Item set ItemCode=?, ItemName=?, ItemCost=?, ItemPrice=?, Category_Name=?, QuantityOnHand=?, deposit=?, Vendor_Name=?, Min_Qty=? where ItemCode=?");
				...
				if(request.getParameter("Category_Name").toString() != "")
        		...
        		UpdateActor.setString(5, Category_Name);
        		...
        		System.out.println(iCode+" "+iName+" "+iCost+" "+iPrice+" "+Category_Name+" "+QuantityOnHand);
